...
/// </summary>
public class MinkaTrigger : MonoBehaviour
{
    ...
    [Tooltip("受け取るオブジェクト")]
    [SerializeField] private GameObject privateHouseObject;
    ...
    [Tooltip("ダジャレのID")]
    [SerializeField] private string punId = "Minka";
    ...
    [SerializeField] private bool shouldDestroyGivenObject = false; // デフォルトfalse
    ...
    private void Awake()
    {
        ...
        if (privateHouseObject != null)
        {
            receiveSpriteRenderers = privateHouseObject.GetComponentsInChildren<SpriteRenderer>();
            receiveOriginalScale = privateHouseObject.transform.localScale;
            ...
            Collider2D receiveCollider = privateHouseObject.GetComponent<Collider2D>();
            if (receiveCollider != null)
            {
                // 受け取るオブジェクトにヘルパースクリプトを追加（既にある場合は追加しない）
                ReceiveTriggerHelper helper = privateHouseObject.GetComponent<ReceiveTriggerHelper>();
                if (helper == null)
                {
                    helper = privateHouseObject.AddComponent<ReceiveTriggerHelper>();
                }
                helper.SetMinkaTrigger(this);
            }
        }
    ...
    private void TriggerPun()
    {
        ...
        {
            Debug.LogWarning($"MinkaTrigger: PunDisplayGeneratorが設定されていません。GameObject: {gameObject.name}");
            return;
        }
